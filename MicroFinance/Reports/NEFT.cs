...
namespace MicroFinance.Reports
{
    public class NEFT : BindableBase
    {
        ...
        public void GenerateNEFT_File_SD(List<CustomerBankDetailView> BankDetails)
        {
            #region Createand WriteHeading
            Excel.Application xlApp = new Microsoft.Office.Interop.Excel.Application();
            Excel.Workbook xlWorkBook;
            ...
            FillNEFT_SD(xlWorkSheet, BankDetails);
            try
            {
                dir = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Report\\NEFT Report\\Security Deposit Neft");
                if (Directory.Exists(dir))
                {
                    string FileName = dir + "\\SD_NEFT_" + DateTime.Now.ToString("dd-MM-yyyy hh-mm") + ".xlsx";
                    xlWorkBook.SaveAs(FileName);
                }
                else
                {
        ...
        public void FillNEFT_SD(Worksheet xlWorkSheet, List<CustomerBankDetailView> BankDetails)
        {
            int i = 1;
            int RowStart = 2;
            foreach (CustomerBankDetailView x in BankDetails)
            {
                // Gtrust Account Details
                xlWorkSheet.Cells[RowStart, 1] = i;
                xlWorkSheet.Cells[RowStart, 2] = RemitterAccNo;
                ...
                //Customer Account Detials
                xlWorkSheet.Cells[RowStart, 5] = x.AccountNumber;
                xlWorkSheet.Cells[RowStart, 6] = x.CustomerName;
                xlWorkSheet.Cells[RowStart, 7] = x.BankBranchName;
                xlWorkSheet.Cells[RowStart, 8] = x.IFSCCode;
                xlWorkSheet.Cells[RowStart, 10] = "Fast";
                xlWorkSheet.Cells[RowStart, 11] = "gtrust@gmail.com";
                xlWorkSheet.Cells[RowStart, 12] = RefNo;
                xlWorkSheet.Cells[RowStart, 13] = x.Amount;
                RowStart++;
                i++;
            }
        }
